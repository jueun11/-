...
function solution(S) {
    ...
    for (let i = 0; i < S.length; ++i) {
        ...
        if (answerArray[answerArrayIdx] === -1) {
            //해당 if문은 전체 배열에서 영어에 해당하는 순서의 요소를 찾는것이다. -1로 된 answerArray[아스키코드-97]가 -1이라는 디폴트 값이라면
            //예를 들어 b가 나왔다 치면. answerArray[1]은 현재 -1이므로
            answerArray[answerArrayIdx] = i;
            //-1로 된 answerArray[아스키코드-97번째 요소]에 i 값을 넣는것이다. 여기서 i는 0부터 단어의 길이까지 점점 커지는 숫자이다.
...
//요소가 26개 있는 배열에서 각 알파벳 아스키 코드를 활용하여(왜냐면 아스키-97을 해도 최대 알파벳 수를 초과하지 않기에), 배열의 순서를 찾아가고(각 알파벳에 해당하는 배열요소), 해당 배열요소에 인덱스값을 집어넣는다!!
